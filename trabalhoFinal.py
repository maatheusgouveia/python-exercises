...
def calcula_assinatura(texto):
    ...
    conjFrases = extrairFrases(conjSentencas)
    conjPalavras = extrairPalavras(conjFrases)
    wal = calcularTamanhoMedioDaPalavra(conjPalavras)
...
def extrairFrases(sentencas):
    frases = []
    for sentenca in sentencas:
        frases += separarFrases(sentenca)
    return frases

def extrairPalavras(sentencas):
    palavras = []
    frases = extrairFrases(sentencas)
    for frase in frases:
        palavras += separarPalavras(frase)
    return palavras

def calcularTamanhoMedioDaPalavra(palavras):
    tamanho = 0
    for pal in palavras:
        tamanho += len(pal)
